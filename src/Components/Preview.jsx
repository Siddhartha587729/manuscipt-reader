...
const Tile = ({ index, item }) => {
  ...
  /* console.log(item) */
  return (
    <div class="min-h-80 xl:h-96 w-1/3 lg:w-1/6 max-w-sm bg-gray-100 rounded-lg flex-col shadow-xl">
      <Link to={`/display/${item._id}`} className='flex-grow '>
        <img class="shadow-md hover:shadow-2xl rounded-t-lg h-1/3 w-full object-cover hover:scale-105" src={url} alt="cover" />
      </Link>
      <div class=" p-5 flex-grow ">
        <Link to="#">
          <h5 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 "><span className='text-gray-500'>Title: </span>{item.title}</h5>
        </Link>
        <div 
        ...
        <Link to={`/display/${item._id}`} class="inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white bg-blue-700 rounded-lg hover:bg-orange-400 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-orange-400 dark:focus:ring-blue-800">
          Read more
